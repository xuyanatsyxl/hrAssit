...
Ext.onReady(function() {
	...
	var GZtbar = new Ext.Toolbar({
		items : [ {
			...
			handler : function() {
				...
				// row.set('qybz', '1'); // 赋初值
				GZgrid.stopEditing();
				GZstore.insert(0, row);
				GZgrid.startEditing(0, 1);
			}
		}, {
			text : '保存',
			iconCls : 'acceptIcon',
			...
			handler : function() {
				...
				var records = GZgrid.getSelectionModel().getSelections();

				if (Ext.isEmpty(records)) {
					Ext.Msg.alert('提示', '你没有选中行');
				}
				Ext.each(records, function(record) {
					GZstore.remove(record);
	...
	// 保存
	function GZsaveRow() {
		var m = GZstore.modified.slice(0); // 获取修改过的record数组对象
		if (Ext.isEmpty(m)) {
			Ext.MessageBox.alert('提示', '没有数据需要保存!');
			return;
		}
		if (!GZvalidateEditGrid(m, 'xmmc')) {
			Ext.Msg.alert('提示', '项目名称字段数据校验不合法,请重新输入!', function() {
				GZgrid.startEditing(0, 2);
			});
			return;
		}
		var jsonArray = [];
	...
	// 检查新增行的可编辑单元格数据合法性
	function GZvalidateEditGrid(m, colName) {
		for ( var i = 0; i < m.length; i++) {
			var record = m[i];
			var rowIndex = GZstore.indexOfId(record.id);
