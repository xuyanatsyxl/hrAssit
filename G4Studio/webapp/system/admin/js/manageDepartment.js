...
Ext
		.onReady(function() {
			...
			var addRoot_adjust = new Ext.tree.AsyncTreeNode({
				text : root_deptname,
				expanded : true,
				id : root_deptid
			});
			var addDeptTree_adjust = new Ext.tree.TreePanel({
				loader : new Ext.tree.TreeLoader({
					baseAttrs : {},
			...
			// 监听下拉树的节点单击事件
			addDeptTree_adjust.on('click', function(node) {
				comboxWithTree_adjust.setValue(node.text);
				Ext.getCmp("adjustFormPanel").findById('parentid_adjust')
						.setValue(node.attributes.id);
				comboxWithTree_adjust.collapse();
			});
			var comboxWithTree_adjust = new Ext.form.ComboBox(
					{
						id : 'deptname_adjust',
						store : new Ext.data.SimpleStore({
							fields : [],
						...
						maxHeight : 390,
						// 下拉框的显示模板,addDeptTreeDiv作为显示下拉树的容器
						tpl : "<tpl for='.'><div style='height:390px'><div id='addDeptTreeAdjustDiv'></div></div></tpl>",
						allowBlank : false,
						onSelect : Ext.emptyFn
					});
			// 监听下拉框的下拉展开事件
			comboxWithTree_adjust.on('expand', function() {
				// 将UI树挂到treeDiv容器
				addDeptTree_adjust.render('addDeptTreeAdjustDiv');
				// addDeptTree.root.expand(); //只是第一次下拉会加载数据
				addDeptTree_adjust.root.reload(); // 每次下拉都会加载数据
			 ...
			 * 调整部门隶属关系
			 */
			var adjustFormPanel = new Ext.form.FormPanel({
				id : 'adjustFormPanel',
				name : 'adjustFormPanel',
				defaultType : 'textfield',
				labelAlign : 'right',
				labelWidth : 65,
				frame : false,
				bodyStyle : 'padding:5 5 0',
				items : [ comboxWithTree_adjust, {
					name : 'deptid',
					hidden : true
				}, {
					id : 'parentid_adjust',
			...
			var adjustWindow = new Ext.Window(
					{
						...
						closeAction : 'hide',
						modal : true,
						title : '<span class="commoncss">调整部门隶属关系</span>',
						// iconCls : 'page_addIcon',
						collapsible : true,
						titleCollapse : true,
						maximizable : false,
						...
						buttons : [
								{
									...
									handler : function() {
										...
										var mode = Ext.getCmp('adjustType')
												.getValue();
										if (mode == 'adjust') {
											saveAdjustDept();
										} else {
			...
			function addInit() {
				...
				var selectModel = deptTree.getSelectionModel();
				var selectNode = selectModel.getSelectedNode();
				Ext.getCmp('parentdeptname').setValue(
						selectNode.attributes.text);
			 ...
			 * 保存调整部门隶属关系
			 */
			function saveAdjustDept() {
				if (!adjustFormPanel.form.isValid()) {
					return;
				}
				adjustFormPanel.form.submit({
					url : './organization.do?reqCode=adjustParentid',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
					...
					failure : function(form, action) {
						var msg = action.result.msg;
						Ext.MessageBox.alert('提示', '部门调整失败:<br>' + msg);
					}
				});
			}
			 ...
			 * 保存合并部门信息
			 */
			function saveMergeDept() {
				if (!adjustFormPanel.form.isValid()) {
					return;
				}
				adjustFormPanel.form.submit({
					url : './organization.do?reqCode=mergeDept',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
					success : function(form, action) {
					...
					failure : function(form, action) {
						var msg = action.result.msg;
						Ext.MessageBox.alert('提示', '部门合并失败:<br>' + msg);
					}
				});
			}
			...
			function adjustInit() {
				var record = grid.getSelectionModel().getSelected();
				if (Ext.isEmpty(record)) {
					Ext.MessageBox.alert('提示', '请先选择要调整的部门!');
					return;
				}
				record = grid.getSelectionModel().getSelected();
				if (record.get('cascadeid') == '001') {
					Ext.MessageBox.alert('提示', '不能调整顶级部门的隶属关系!');
					return;
				}
				adjustFormPanel.getForm().loadRecord(record);
				adjustWindow.show();
				adjustWindow
						.setTitle('<span style="font-weight:normal">调整部门隶属关系</span>');
				Ext.getCmp('parentid_old_adjust').setValue(
						record.get('parentid'));
				Ext.getCmp('adjustType').setValue('adjust');
			}
			...
			function mergeInit() {
				var record = grid.getSelectionModel().getSelected();
				if (Ext.isEmpty(record)) {
					Ext.MessageBox.alert('提示', '请先选择要合并的部门!');
					return;
				}
				record = grid.getSelectionModel().getSelected();
				if (record.get('leaf') == '0') {
					Ext.MessageBox.alert('提示', '只能合并末级部门!');
					return;
				}
				adjustFormPanel.getForm().loadRecord(record);
				adjustWindow.show();
				adjustWindow
						.setTitle('<span style="font-weight:normal">合并部门</span>');
				Ext.getCmp('parentid_old_adjust').setValue(
						record.get('parentid'));
				Ext.getCmp('adjustType').setValue('mergin');
			}
