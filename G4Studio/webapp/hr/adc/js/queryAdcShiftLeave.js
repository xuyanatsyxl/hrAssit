	...
	comboxWithTree.on('expand', function() {
		...
		addDeptTree.root.reload(); // 每次下拉都会加载数据
			...
			var cm = new Ext.grid.ColumnModel([rownum, {
				header : '值班日期', // 列标题
				dataIndex : 'adc_date', // 数据索引:和Store模型对应
				width : 120,
				sortable : true
			}, {
				header : '部门编码',
			...
			var store = new Ext.data.Store({
						...
						proxy : new Ext.data.HttpProxy({
									url : './adcshiftleave.do?reqCode=reportAdcShiftLeave'
								}),
						// 数据读取器
			...
			var grid = new Ext.grid.GridPanel({
						...
						title : '<span class="commoncss">综合业务查询</span>',
						// height : 500,
						autoScroll : true,
						frame : true,
						store : store, // 数据存储
