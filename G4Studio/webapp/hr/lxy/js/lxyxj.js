...
Ext.onReady(function() {
			...
			var grid = new Ext.grid.GridPanel({
				...
				title : '<span class="commoncss">联销员星级查询</span>',
				// height : 500,
				autoScroll : true,
				frame : true,
				store : store, // 数据存储
			...
			function editXJinit(){
			 ...
			 var xjnxComboB=new Ext.form.ComboBox({
					hiddenName : 'xjnx',
					fieldLabel : '星级年限',
					emptyText : '请选择',
					triggerAction : 'all',
			...
			var xjForm = new Ext.form.FormPanel({
				...
				items : [YGXJCombo1,
				         {		fieldLabel : '获取星级时间', // 标签
							  ...
							  {
								fieldLabel : '星级有效期至', // 标签
								xtype : 'datefield',
								format : 'Y-m-d',
								name : 'xjjsrq', // name:后台根据此name属性取值
								allowBlank : false,
								labelStyle : 'color:blue;',
								anchor : '100%'// 宽度百分比	
							}*/,xjnxComboB,
			...
			function editlxyxj(){
				...
				//选择有星级必须选有效期
				if ((YGXJCombo1.getValue() >= '1') && Ext.isEmpty(xjnxComboB.getValue())){
					Ext.Msg.alert('错误', '有星级必须选择有效期！');
					return;
				}
				...
				xjForm.form.submit({
					url : './lxy.do?reqCode=updatelxyxj',
					waitTitle : '提示',
					method : 'POST',
					waitMsg : '正在处理数据,请稍候...',
