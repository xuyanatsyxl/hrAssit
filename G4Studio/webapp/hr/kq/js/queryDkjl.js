...
Ext.onReady(function() {
	...
	var store = new Ext.data.Store({
		...
		proxy : new Ext.data.HttpProxy({
			url : 'dkj.do?reqCode=queryDkjlForManage'
		}),
		// 数据读取器
	...
	var tbar = new Ext.Toolbar({
		items : [{
			...
			handler : function() {
				if (store.getCount() > 0) {
					exportExcel('dkj.do?reqCode=queryDkjlForExcel');
	...
	var grid = new Ext.grid.GridPanel({
		...
		title : '<span class="commoncss">打卡流水查询</span>',
		// height : 500,
		autoScroll : true,
		frame : true,
		store : store, // 数据存储
