...
Ext.onReady(function() {
	...
	addDeptTree.on('click', function(node) {
		...
		Ext.getCmp("firstForm").findById('deptid')
	...
	var store = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
							url : './adcshiftupload.do?reqCode=queryAdcShiftUploadItemForManage'
						}),
	...
	function submitTheForm() {
		...
		var requesturl = './adcshiftupload.do?reqCode=doUpload';
		if (mode == 'ftp') {
	...
	function delFiles() {
		...
		var strChecked = jsArray2JsString(rows, 'upfileid');
		Ext.Msg.confirm('请确认', '你真的要删除选中的文件吗?', function(btn, text) {
					if (btn == 'yes') {
						showWaitMsg();
						Ext.Ajax.request({
									url : './adcshiftupload.do?reqCode=delFiles',
