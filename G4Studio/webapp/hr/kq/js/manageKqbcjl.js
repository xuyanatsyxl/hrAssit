		...
		.onReady(function() {
			...
			var firstStore = new Ext.data.Store({
				proxy : new Ext.data.HttpProxy({
					url : 'kqbc.do?reqCode=queryWdkyy'
				}),
				...
				buttons : [
						{
							...
							iconCls : 'previewIcon',
							handler : function() {
								if (!Ext.getCmp('qForm').findById('deptid')
										.getValue()) {
			...
			var store = new Ext.data.Store({
				// 获取数据的方式
				proxy : new Ext.data.HttpProxy({
					url : 'kqbc.do?reqCode=queryKqbcjlForManage'
				}),
				// 数据读取器
				reader : new Ext.data.JsonReader({
			...
			var tbar = new Ext.Toolbar({
				items : [ {
					...
					handler : function() {
						if (store.getCount() > 0) {
							exportExcel('kqbc.do?reqCode=queryKqbcjlForExcel');
						...
						buttons : [
								{
									...
									handler : function() {
										...
										procDataForm.form
												.submit({
													url : './kqbc.do?reqCode=updateKqbcjlItem',
													waitTitle : '提示',
													method : 'POST',
													waitMsg : '正在处理数据,请稍候...',
