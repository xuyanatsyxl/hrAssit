...
function doExport(pFlag, pWidth, pHeight) {
	...
	var str = 'width='
			...
			+ ",left="
			+ left
			+ ',toolbar=no,menubar=no,scrollbars=no,resizable=yes,location=no,status=no,fullscreen=yes';
	window.open(webContext + initUrl, '', str);
}
 ...
 * 初始化HTML导出窗口
 */
function doHtmlExport(pFlag, pWidth, pHeight) {
	var initUrl = '/report.do?reqCode=initHtmlPage';
	if (!Ext.isEmpty(pFlag))
		initUrl = initUrl + '&flag=' + pFlag;
	if (Ext.isEmpty(pWidth))
		pWidth = 800;
